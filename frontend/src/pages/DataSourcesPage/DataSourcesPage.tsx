    ...
    <div className={`${styles.page} page-enter`}>
      ...
      >
        <div className={styles.uploadCardHeader}>
          ...
          <div>
            ...
            <p className={styles.uploadCardSub}>Select the Portal, Upload Your File — Data Merges Instantly into the Dashboard</p>
          </div>
        </div>
                ...
                <motion.div className={styles.sourceDropMenu} initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.15 }}>
                  {sources.map(src => (
                    <button
                      ...
                      onClick={() => { setSelectedSource(src); setSourceDropOpen(false); setUploadError(''); if (src.key !== 'others') setCustomName(''); }}
                    >
      ...
      <div className={styles.historySection}>
        <div className={styles.historyHeader}>
          ...
          <button className={styles.refreshBtn} onClick={() => void refetch()}><RefreshCw size={13} /> Refresh</button>
        </div>
