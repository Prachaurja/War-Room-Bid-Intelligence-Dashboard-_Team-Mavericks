...
const INITIAL_ALERTS: Alert[] = [
  { id:'ALT-001', title:'New Facility Management Tender — NSW',        description:'A $4.2M facility management contract from NSW Public Works has been published matching your saved search criteria.',     type:'tender', priority:'high',   timestamp: new Date(Date.now()-30*60000).toISOString(),    read:false },
  { id:'ALT-002', title:'Bid Deadline Approaching — Healthcare VIC',   description:'Your tracked bid for Victorian Health Department IT infrastructure closes in 48 hours. Review and finalise submission.', type:'bid',    priority:'high',   timestamp: new Date(Date.now()-2*3600000).toISOString(),   read:false },
  { id:'ALT-003', title:'New Cleaning Contract — QLD Government',      description:'3 new cleaning and hygiene contracts published in Queensland totalling $1.8M across regional health facilities.',         type:'tender', priority:'medium', timestamp: new Date(Date.now()-5*3600000).toISOString(),   read:false },
  { id:'ALT-004', title:'Weekly Intelligence Report Ready',            description:'Your weekly bid intelligence report covering 47 new tenders across all states is now available for download.',              type:'report', priority:'low',    timestamp: new Date(Date.now()-8*3600000).toISOString(),   read:true  },
  { id:'ALT-005', title:'AusTender Ingestion Complete',                description:'Scheduled data ingestion completed successfully. 500 contracts processed, 12 new records added to your database.',         type:'system', priority:'low',    timestamp: new Date(Date.now()-12*3600000).toISOString(),  read:true  },
  { id:'ALT-006', title:'High-Value Tender Alert — Defence Housing',   description:'A $24.5M facility management opportunity from Defence Housing Australia matches your VIP client David Chen.',              type:'tender', priority:'high',   timestamp: new Date(Date.now()-24*3600000).toISOString(),  read:true  },
  { id:'ALT-007', title:'Saved Search Match — IT Services ACT',        description:'Your "IT Services Canberra" saved search matched 2 new contracts from the ACT Government Digital team.',                   type:'tender', priority:'medium', timestamp: new Date(Date.now()-36*3600000).toISOString(),  read:true  },
  { id:'ALT-008', title:'Bid Status Update — Ausgrid',                 description:'Your bid for Ausgrid infrastructure maintenance has progressed to shortlist stage. Prepare for presentation.',             type:'bid',    priority:'medium', timestamp: new Date(Date.now()-48*3600000).toISOString(),  read:true  },
];

const INITIAL_SEARCHES: SavedSearch[] = [
  { id:'SS-001', name:'Facility Mgmt — NSW',      sector:'facility_management', state:'NSW', minValue:500000,  maxValue:10000000, notifications:true,  lastMatched:'30 min ago', matchCount:4  },
  { id:'SS-002', name:'Cleaning — All States',    sector:'cleaning',            state:'',    minValue:100000,  maxValue:5000000,  notifications:true,  lastMatched:'5h ago',     matchCount:12 },
  { id:'SS-003', name:'IT Services — ACT',        sector:'it_services',         state:'ACT', minValue:1000000, maxValue:20000000, notifications:false, lastMatched:'1d ago',     matchCount:2  },
  { id:'SS-004', name:'Construction — QLD + VIC', sector:'construction',        state:'',    minValue:2000000, maxValue:50000000, notifications:true,  lastMatched:'3d ago',     matchCount:8  },
];
...
const SECTOR_LABELS: Record<string, string> = {
  ...
  healthcare:'Healthcare', transportation:'Transportation', other:'Other', '':'All Sectors',
};

// ── Create alert modal ────────────────────────────────────────
function CreateAlertModal({ onClose, onAdd }: {
  onClose: () => void;
  onAdd:   (s: SavedSearch) => void;
}) {
  ...
  const handleSave = () => {
    ...
    onAdd({
      ...
      maxValue:      Number(form.maxValue) || 0,
...
export default function AlertsPage() {
  ...
  const statCards = [
    ...
    { label:'Notifications', value:Object.values(notifPrefs).filter(Boolean).length + '/3', sub:'Channels Active', color:'#10B981', bg:'rgba(16,185,129,0.1)', icon:Mail },
  ];
  ...
  return (
    <div className={`${styles.page} page-enter`}>
            ...
            <div className={styles.filterRow}>
              ...
              <div className={styles.filterRight}>
                ...
                {alerts.some(a => a.read) && (
                  <button className={styles.clearReadBtn} onClick={clearRead}>
                    ...
                    >
                      ...
                      <div className={styles.alertContent}>
                        <div className={styles.alertTopRow}>
                          ...
                          <span className={clsx(styles.priorityBadge, pc.cls)}>
                            {pc.label}
                          </span>
                        </div>
                        ...
                        <div className={styles.alertMeta}>
                          <Clock size={11} />
                          <span>{formatAgo(alert.timestamp)}</span>
                ...
                {searches.map((s, i) => (
                  ...
                  >
                    ...
                    <div className={styles.ssCriteria}>
                      ...
                      {(s.minValue > 0 || s.maxValue > 0) && (
                        <div className={styles.ssCrit}>
                          <DollarSign size={10} />
                          {s.minValue > 0 ? `$${(s.minValue/1000000).toFixed(1)}M` : '—'}
                          {' – '}
                          {s.maxValue > 0 ? `$${(s.maxValue/1000000).toFixed(1)}M` : '—'}
                        </div>
                      )}
                    </div>
                    ...
                    <div className={styles.ssMeta}>
                      ...
                      <span className={styles.ssMatch}>
                        {s.matchCount} matches · {s.lastMatched}
                      </span>
                    </div>
                  </motion.div>
                ))}
      ...
      <AnimatePresence>
        {showModal && (
          <CreateAlertModal onClose={() => setShowModal(false)} onAdd={addSearch} />
        )}
      </AnimatePresence>
